 ...
 * This is a version of the linear regression program that uses 8 MPI instances, 
 * to each compute the error associated with a specific regression line. The master 
 * instance runs the main algorithm and uses a different instance to compute the 
 * error associated with each m, c being explored

 * To compile:
 *   mpicc -o 3_a_MPILinearRegression 3_a_MPILinearRegression.c -lm
 * 
 * To run:
 *   mpirun -n 9 ./3_a_MPILinearRegression
 * 
